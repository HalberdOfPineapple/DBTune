...
def parse_job(file_path, select_file, timeout=4):
    # lines - lines of the output file (query_name, query_latency)
    with open(file_path) as f:
        lines = f.readlines()
    ...
    # num_sql - number of SQL statements executed in total
    num_sql = len(lines_select)

    # latL - list of latency data from the output file
    latL = []
    for line in lines[1:]:
        if line.strip() == '':
            continue
    ...
    # measured_num - number of query files being executed
    # assume other lines (not executing the actual query) taking 4s each
